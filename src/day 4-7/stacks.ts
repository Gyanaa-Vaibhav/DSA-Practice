...
function validParentheses(string:string){
    const stack = new Stack<string>()
...
function sortUsingAnotherStack(array:number[]) {
    const stack = new Stack<number>()
...
function reversePolishNotation(array:string[]){
    const stack = new Stack<string>()
...
function stockSpan(prices: number[]) {
    const stack = new Stack<number>(); // Stack to store indices
    const spans: number[] = new Array(prices.length).fill(1); // Default span = 1 for each day
