...
function secondSmallestAndSecondLargest(array:number[]) {
    let max = -Infinity;
    let sMax = -Infinity + 1;
    let min = Infinity;
    let sMin = Infinity - 1;
...
function equilibriumIndex(array:number[]) {
    let leftSum = 0
...
function maximProductOfTwoIndex(array:number[]) {
...
function mostFrequentElement(array:number[]){
    const map = {}
    let maxCount = 0
    let key:number;
    for (let i = 0; i < array.length; i++) {
...
function twoSum(array:number[],k:number) {
    const map = {}
...
function twoSumClosest(array:number[],k:number) {
    ...
    for (let i = 1; i < array.length; i++) {
        if(array[i - 1] <= closest && array[i] >= closest){
            if(array[i] >= closest) ans = [array[i],k-closest]
            if(array[i - 1] <= closest) ans = [array[i - 1],k-closest]
        }
    }
...
function longestSubStringWithoutRepeatingCharacters(string:string){
    let length = 0;
    let maxLength = 0;
    const map = {};
